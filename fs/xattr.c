...
static void
fix_xattr_to_user(struct user_namespace *mnt_userns, const char *kname,
		  void *kvalue, size_t size)
{
	if (strncmp(kname, XATTR_SYSTEM_PREFIX, XATTR_SYSTEM_PREFIX_LEN))
		return;
	kname += XATTR_SYSTEM_PREFIX_LEN;
	if ((strcmp(kname, XATTR_POSIX_ACL_ACCESS) == 0) ||
	    (strcmp(kname, XATTR_POSIX_ACL_DEFAULT) == 0))
		posix_acl_fix_xattr_to_user(mnt_userns, kvalue, size);
}
	 ...
	 const char __user *name, void __user *value, size_t size)
{
	...
	if (error > 0) {
		fix_xattr_to_user(mnt_userns, kname, kvalue, error);
